#!/bin/bash
#
# link-dev.sh - Development linking script for opentui-spinner
#
# This script creates a symbolic link (or copy) of the opentui-spinner package
...
echo "Linking opentui-spinner from: $REPO_ROOT"
...
# Link opentui-spinner
remove_if_exists "$NODE_MODULES_DIR/opentui-spinner"
PACKAGE_PATH="$REPO_ROOT$SUFFIX"
if [ -d "$PACKAGE_PATH" ]; then
	link_or_copy "$PACKAGE_PATH" "$NODE_MODULES_DIR/opentui-spinner" "opentui-spinner"
else
